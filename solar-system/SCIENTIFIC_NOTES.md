...
The SDO/HMI Sun layer is one fixed 2025-12-26 observer-facing continuum observation, not a global spherical map. Runtime extracts its supported active-region contrast and applies that detail over a continuous procedural photosphere instead of using the image's absolute color as one hemisphere. The unobserved side remains procedural. Sun rotation in the application does not turn this image into a contemporaneous full-Sun data product, and the texture is not updated with the simulation date.
...
Saturn's sampled Cassini wind table contains documented gaps. The project linearly bridges `+5.5°` to `−6.5°`, `−40.5°` to `−43.5°`, and `−54°` to `−57.5°`; motion within those spans is interpolation, not measurement. Saturn's Hubble OPAL Cycle 32 Rotation A color map is a dated F395N/F502N/F631N composite from 2025-08-29, not measured albedo or live weather. The runtime derivative uses alpha coverage to reject black/no-data polar rows, the ring-obscured equatorial strip, and isolated transiting-body artifacts, then blends the remaining observation at up to `86%` over procedural bands. Those fallback pixels and their motion are project-authored rather than recovered observations. Jupiter's measured table ends at `±60°`, so the project tapers the profile to zero at each pole. The Uranus conversion and Neptune polynomial use their declared reference rotations and sign conventions, which are not substitutes for authoritative prime-meridian rotation series.
...
The Sun's base photosphere remains deterministic shader noise. High and ultra quality add finer multiscale granulation and dark intergranular lanes; the same procedural field authors structured penumbra, umbra, and limb-weighted faculae. The dated HMI continuum disk contributes only active-region contrast on its supported hemisphere, leaving the base color and exposure continuous around the sphere. A lower `1.48` display-emission multiplier preserves more photosphere contrast, and the quality-gated corona shells fade progressively outward. The morphology was reviewed against the official [SDO HMI/AIA channel descriptions](https://sdo.gsfc.nasa.gov/data/channels.php) and NASA SVS's [February 2013 *Busy Sun* HMI visible-light sequence and 4K frames](https://svs.gsfc.nasa.gov/4133/). The result is not a live or timestamp-matched Sun, radiometrically calibrated imagery, a measured full-Sun record, or a magnetohydrodynamic model.
