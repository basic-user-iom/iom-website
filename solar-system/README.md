...
## Observatory asset and catalog refresh

- Complete-catalog search now includes planets, natural moons, the five bundled Earth-orbit objects, and fourteen spacecraft. A result enables its hidden layer, selects the object, and frames its location; the dedicated Natural satellites panel also searches across every parent system.
- Sixteen major moons use 2048 x 1024 WebP globe maps extracted from official NASA VTAD 3D resources: Io, Europa, Ganymede, Callisto, Enceladus, Tethys, Dione, Rhea, Titan, Iapetus, Miranda, Ariel, Umbriel, Titania, Oberon, and Triton. Earth's Moon keeps its existing LRO map. The remaining seven rendered major moons use distinct deterministic procedural coverage because no compatible official equirectangular map was bundled. In particular, the official Mimas and Hyperion model images are mesh-specific UV atlases and are deliberately not stretched onto spheres.
- The Sun combines one continuous procedural animated photosphere/corona with active-region contrast extracted from a fixed 2048-pixel NASA SDO/HMI continuum observation from 2025-12-26. The observation is an observer-facing disk rather than a global map, so its absolute color is not wrapped over one hemisphere: only its supported sunspot morphology is softly applied over the seamless modeled surface. It is neither live imagery nor a fabricated global map.
- Presentation-scale major-moon radii preserve each moon-to-parent physical radius ratio after applying the parent's display exaggeration. A small contextual minimum keeps selected moons usable, with a strict 6.5% parent-radius cap; minor and artificial-satellite markers are much smaller and remain explicitly not to scale.
- Exact source URLs, transforms, dimensions, bytes, and SHA-256 hashes for these 17 new derivatives are recorded in [`public/assets/moons/manifest.json`](./public/assets/moons/manifest.json). `npm run space-objects:moon-textures:verify` audits the complete bundle offline.
...
- Screen-space labels use collision suppression with priority for the selected object. The searchable object navigator can focus all 15 bodies and directly navigate the natural-moon, Earth-satellite, and spacecraft catalogs; it also toggles orbit lines, labels, sky, bright stars, comets, or either statistical belt. The inspector reports catalog properties plus current heliocentric distance and speed.
...
- The body renderer lazily loads the 16 Phase 4/5 compact runtime textures plus the separately manifested SDO/HMI Sun observation. Seven Phase 4/5 files are byte-for-byte official NASA/USGS color, radar, night, or cloud downloads and nine are deterministic project-generated derivatives. Phase 5 uses lossless Hubble OPAL global-map derivatives for Jupiter and Saturn plus a local Jupiter luminance-detail WebP derived from NASA JunoCam PIA23606. Saturn's coverage alpha lets black/no-data, ring-obscured, and isolated artifact pixels fall back to its continuous procedural wind layer. Load failure remains visible in diagnostics and falls back to deterministic procedural materials. Exact Phase 4/5 runtime/source hashes, byte lengths, dimensions, transformations, linear/sRGB handling, and semantics are recorded in `public/assets/source-manifest.json`; the Sun and major-moon additions are recorded separately in `public/assets/moons/manifest.json`.
...
- The Sun uses quality-gated procedural granulation, dark intergranular lanes, structured spot/facula cues, limb darkening, and a layered corona. Dated SDO/HMI sunspot contrast is added without replacing the base color of either hemisphere, preventing an artificial join between the observed and modeled regions. The far side remains procedural and the result is not live or timestamp-matched solar weather. Venus remains procedural: its two cloud decks share one modeled four-day westward superrotation and add restrained UV-absorber-inspired contrast, chevrons, zonal structure, a polar hood, and softer twilight informed qualitatively by [Hubble UV cloud-top imagery](https://science.nasa.gov/photojournal/venus-cloud-tops-viewed-by-hubble/) and [Akatsuki UVI](https://darts.isas.jaxa.jp/missions/akatsuki/uvi_en.html). No UVI texture or cloud-motion-vector product drives the current renderer, so this is not observed or live Venus weather; the separate Magellan radar **data view** remains available beneath the clouds. Earth combines Blue Marble, independent city-light and cloud composites, a separately rotating cloud shell, separate ocean glint/roughness inputs, and terminator-gated lights. Mars combines mapped color/topography with procedural polar caps, dust clouds, and haze.
...
Phase 4 visual assets live in `public/assets/phase4/`; the Phase 5 Jupiter and Saturn maps plus local GRS detail live in `public/assets/phase5/`; Phase 6 sky assets and their dedicated manifest live in `public/assets/phase6/`; the Sun observation and major-moon maps live in `public/assets/moons/`. Machine-readable provenance and SHA-256 digests are split between `public/assets/source-manifest.json`, `public/assets/phase6/sky-manifest.json`, and `public/assets/moons/manifest.json`. Human-readable provenance and scientific caveats live in [SOURCES.md](./SOURCES.md) and [SCIENTIFIC_NOTES.md](./SCIENTIFIC_NOTES.md).
...
npm run space-objects:moon-textures:verify
npm run build
```
...
- [Machine-readable Sun and major-moon texture manifest](./public/assets/moons/manifest.json)
