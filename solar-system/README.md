...
- The body renderer lazily loads the 15 Phase 4/5 compact runtime textures plus the separately manifested SDO/HMI Sun observation. Seven Phase 4/5 files are byte-for-byte official NASA/USGS color, radar, night, or cloud downloads and eight are deterministic project-generated derivatives. Phase 5 uses one lossless WebP generated without resizing or recoloring from Hubble OPAL's December 2025 Rotation A global Jupiter TIFF and one local luminance-detail WebP derived from NASA JunoCam PIA23606. Load failure remains visible in diagnostics and falls back to deterministic procedural materials. Exact Phase 4/5 runtime/source hashes, byte lengths, dimensions, transformations, linear/sRGB handling, and semantics are recorded in `public/assets/source-manifest.json`; the Sun and major-moon additions are recorded separately in `public/assets/moons/manifest.json`.
...
Phase 4 visual assets live in `public/assets/phase4/`; the Phase 5 Jupiter map and local GRS detail live in `public/assets/phase5/`; Phase 6 sky assets and their dedicated manifest live in `public/assets/phase6/`; the Sun observation and major-moon maps live in `public/assets/moons/`. Machine-readable provenance and SHA-256 digests are split between `public/assets/source-manifest.json`, `public/assets/phase6/sky-manifest.json`, and `public/assets/moons/manifest.json`. Human-readable provenance and scientific caveats live in [SOURCES.md](./SOURCES.md) and [SCIENTIFIC_NOTES.md](./SCIENTIFIC_NOTES.md).
