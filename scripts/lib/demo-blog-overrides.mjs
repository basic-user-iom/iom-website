...
**Download (Windows x64):** [Setup](https://github.com/basic-user-iom/3d/releases/download/v3.19.2/3D-Viewer-Setup-3.19.2-x64.exe) | [Portable](https://github.com/basic-user-iom/3d/releases/download/v3.19.2/3D-Viewer-Portable-3.19.2-x64.exe) | [Release notes](https://github.com/basic-user-iom/3d/releases/tag/v3.19.2)`,
  ...
  'raven-path': {
    ...
    excerpt:
      'Animate a raven (or your own GLB) along an editable spline — export path JSON for other software, reimport next visit, and tune timing in the browser.',
    seo_title: 'Raven Path Animation — spline flight & path export — IOM',
    seo_description:
      'Try IOM’s Raven Path demo: editable Catmull-Rom flight, GLB/GLTF/FBX import, path JSON export/reimport, and a beginner walkthrough in the 3D section.',
    ...
    glossary: [
      { term: 'Catmull-Rom spline', def: 'a smooth curve that passes through control points ([Wikipedia](https://en.wikipedia.org/wiki/Centripetal_Catmull%E2%80%93Rom_spline))' },
      ...
      { term: 'Tangent-aligned', def: 'the model turns to face along the path direction' },
      { term: 'Skeletal animation', def: 'bones drive secondary motion (like wing flaps) while the root follows the curve' },
    ],
    trySteps: [
      'Open the [Raven Path demo](/demos/raven-path/)',
      'Watch one lap, then drag a spline control point and see the route reshape',
      'In **Path**: **Export path JSON**, then **Import path JSON** (or drag the file onto the scene)',
      'Optional: **Import GLB / GLTF / FBX**, then tune speed, ease, reverse, or tangent orientation',
    ],
    requirements: [
