...
export const DEMO_BLOG_OVERRIDES = {
  ...
  'raven-path': {
    ...
    hook: 'Sometimes the story is the motion, not the still. Raven Path puts a winged GLB on a Catmull-Rom spline — drag control points, tune speed and easing, reverse the route, and keep wing-flap animation playing while the bird follows the path. Bring your own GLB, GLTF, or FBX onto the path, and export or reimport path JSON for other software or your next session on the site.',
    coverNote: 'The cover shows the raven on its editable flight path in the 3D section.',
    whyBullets: [
      '- **Path as a design tool** — reshape the flight with visible control points',
      '- **Import your own model** — drop in GLB, GLTF, or FBX to replace the stock raven on the path',
      '- **Export & reimport the path** — download path JSON for other tools, or load it again next visit',
    ...
    whyUses:
      'hero loops for brand films, booth attract loops, narrative web chapters, prototyping creature or product “travel” paths before a full animation pass, and handing a reusable path JSON to other pipelines.',
    beginner:
      'A spline is a smooth curve defined by a few handles — like a flexible wire in space. Here a raven (or your imported model) rides that wire. You pull the handles, and the flight updates live. Export the path as JSON when you want to reuse it later or in other software; import it back when you return.',
    glossary: [
      { term: 'Catmull-Rom spline', def: 'a smooth curve that passes through control points' },
      { term: 'GLB / GLTF / FBX', def: 'common 3D model formats you can import onto the path' },
      { term: 'Path JSON', def: 'exported control points (and options) you can reimport on the site or use elsewhere' },
    ...
    trySteps: [
      'Open the [Raven Path demo](/demos/raven-path/) in the [3D section](/#3d)',
      'Watch the raven follow the default path once',
      'Drag a spline control point and see the route reshape',
      'Use **Export path JSON**, then **Import path JSON** (or drag the file onto the scene) to confirm round-trip',
      'Optional: **Import GLB / GLTF / FBX** to put your own model on the path',
    ...
    requirements: [
      ...
      '**Files:** path export is JSON; models should be self-contained GLB (or FBX) when possible',
    ],
    ...
    alsoCan: [
      ...
      'Browse sibling realtime scenes in the [3D section](/#3d), like [Volumetric Lighting](/demos/volume-lighting/)',
    ],
    howWorks:
      'The demo uses [Three.js](https://threejs.org/) to sample a Catmull-Rom curve each frame, place the model root on that sample, and optionally align its forward axis to the curve tangent while a skeletal clip (when present) drives secondary motion. Path JSON stores control points, closed-loop, and path transform so you can reimport on [iobjectm.com](/demos/raven-path/) or feed the points into other software. Same family of ideas as three.js curve and animation examples — tuned here for a readable creature loop with import/export.',
    faq: [
      {
        q: 'Can we swap the raven for our mascot?',
        a: 'Yes — use **Import GLB / GLTF / FBX** in the demo to try your model on the path right away. For a branded production build, ask us for a scoped version.',
      },
      {
        q: 'How do I reuse a path later or in other software?',
        a: 'Use **Export path JSON** in the Path panel. Reimport that file next time on the site, or use the `points` / `threeJsSnippet` fields in Blender, Three.js, or your own tools.',
      },
      {
      ...
      {
        q: 'Where is this on the homepage?',
        a: 'Under the [3D section](/#3d) as **Raven Path Animation**.',
      },
    ],
    reading: [
      { label: 'Raven Path demo', url: '/demos/raven-path/' },
      { label: '3D section on the homepage', url: '/#3d' },
      { label: 'Three.js', url: 'https://threejs.org/' },
      {
      ...
      { label: 'Volumetric Lighting (model import sibling)', url: '/demos/volume-lighting/' },
    ],
    related: [
      { label: 'Volumetric Lighting — Rect Area', url: '/blog/volume-lighting' },
