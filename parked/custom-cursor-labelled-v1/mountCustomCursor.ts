import { subscribeCursorApi } from './api'
 ...
 * Avoids React state on mousemove for performance.
 */
export function mountCustomCursor(): (() => void) | null {
  if (!isCustomCursorSupported()) return null

  // Replace any prior instance (Strict Mode remount / HMR).
  if (activeCleanup) {
    activeCleanup()
    activeCleanup = null
  }
  ...
  const applyResolved = (next: ResolvedCursor) => {
    if (modesEqual(current, next) && dom.root.classList.contains(VISIBLE_CLASS) === visible) {
      // Still refresh press/drag classes below when only those change.
    }
