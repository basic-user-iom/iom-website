...
export function renderReceivedEmailHtml(opts: {
  subject: string
  body: string
}): string {
  const subject = escapeHtml(opts.subject || '(no subject)')
  const bodyHtml = bodyToHtml(opts.body)
  return `<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="background-color:#ececf2;">
    <tr>
      <td align="center" style="padding:28px 16px;">
        <table role="presentation" width="600" cellpadding="0" cellspacing="0" border="0" style="width:100%;max-width:600px;background-color:#f7f7fa;border:1px solid #d8d8e0;">
          <tr>
            <td style="padding:18px 28px;background-color:#0a1420;border-bottom:1px solid rgba(0,229,255,0.18);">
              <p style="margin:0;font-family:${FONT_BODY};font-size:11px;font-weight:600;letter-spacing:0.08em;text-transform:uppercase;color:#00e5ff;">Received email</p>
              <p style="margin:8px 0 0;font-family:${FONT_DISPLAY};font-size:18px;font-weight:700;line-height:1.3;color:#ececf2;">${subject}</p>
            </td>
          </tr>
          <tr>
            <td style="padding:28px 32px 32px;">${bodyHtml || `<p style="margin:0;font-family:${FONT_BODY};font-size:15px;color:#6a6a78;">(empty)</p>`}</td>
          </tr>
        </table>
      </td>
    </tr>
...
/** Formatted HTML for a stored thread message. */
export function htmlForLeadMessage(msg: {
  direction: 'outbound' | 'inbound'
  subject: string
  body_text: string
  body_html: string | null
}): string {
  const stored = msg.body_html?.trim() || ''
  if (stored) {
    if (/<html[\s>]/i.test(stored) || /<body[\s>]/i.test(stored)) return stored
    const subject = escapeHtml(msg.subject || '(no subject)')
    return `<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"><title>${subject}</title></head><body style="margin:0;padding:24px;background:#ececf2;font-family:${FONT_BODY};color:#1c1c22;">${stored}</body></html>`
  }
  if (msg.direction === 'outbound') {
    return renderOutreachEmailHtml({
      subject: msg.subject,
      body: msg.body_text,
    })
  }
  return renderReceivedEmailHtml({
    subject: msg.subject,
    body: msg.body_text,
  })
}
